...
// Retry function for video uploads
const uploadVideoWithRetry = async (pageId, pageAccessToken, videoBuffer, caption = '', retries = 3) => {
    const formData = new FormData();
    formData.append("source", videoBuffer, {
        filename: "video.mp4",
    ...
    while (attempt < retries && !uploadSuccess) {
        try {
            ...
            console.error(`Error uploading video (attempt ${attempt + 1}):`, error);
            attempt++;
            if (attempt < retries) {
                console.log(`Retrying video upload...`);
            }
        }
    }
    ...
    try {
        ...
        if (postType === 'feed') {
            ...
            if (files && files.length > 0) {
                ...
                (async () => {
                    try {
                        const videoId = await uploadVideoWithRetry(pageId, pageAccessToken, videoBuffer, caption);
                        const postId = await createVideoPost(pageId, pageAccessToken, videoId);
                        console.log('Video uploaded and post created successfully:', postId);
                    } catch (error) {
                        console.error('Error during video upload or post creation:', error.message);
                    }
                })();
